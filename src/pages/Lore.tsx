...
const Lore = () => {
  ...
  const fetchPosts = async () => {
    try {
      const data = await postService.getPosts('lore');
      setPosts(data);
    } catch (error) {
      console.error('Error fetching posts:', error);
      alert('Failed to fetch lore posts. Please check your internet connection.');
    } finally {
      setLoading(false);
    }
  };
    ...
    return (
      <div className="pt-16 min-h-screen flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-gray-300">Loading lore from cloud...</p>
        </div>
      </div>
    );
  }
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        ...
        <div className="text-center mb-12 fade-in">
          <div className="flex justify-center mb-6">
            <Bookmark className="h-16 w-16 text-purple-400 pulse-glow" />
          </div>
          <h1 className="text-5xl font-bold gradient-text heading-font mb-4 pb-2">
            Lore & Story
          </h1>
          <p className="text-xl text-gray-300 max-w-3xl mx-auto">
            Look into the rich history and eternal story of the Electro Archon
          </p>
        </div>
        ...
        {posts.length > 0 ? (
          ...
          <div className="text-center py-20">
            ...
            <h3 className="text-2xl font-bold text-gray-400 mb-2">No lore posts yet</h3>
            <p className="text-gray-500 mb-8">
              The archive is empty. Visit the dashboard to add your first lore entry.
            </p>
            <Link
              to="/dashboard"
